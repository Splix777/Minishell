...
void    open_fds(t_minishell *minishell, t_command *cmd)
{
    ...
    while (redir)
    {
        ...
        if (redir->next && !error && (redir->next->type == REDIR_IN || redir->next->type == HEREDOC))
            close(cmd->fdin);
        if (redir->next && !error && (redir->next->type == REDIR_OUT || redir->next->type == APPEND))
            close(cmd->fdout);
        if (cmd->fdin < 0 && !error)
        {
            error = 1;
